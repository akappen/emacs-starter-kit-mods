...
(defun do-run-spec (&rest args)
  (redo-spec (cons 'do-run-spec args))
  (setq rspec-results (get-buffer-create "*rspec-results*"))
  (save-excursion
    (set-buffer rspec-results)
    (erase-buffer)
    (setq linkify-regexps '("^    # \\(.*?\\):\\([0-9]*\\).*$")))
  (setq proc (apply #'start-process "rspec" rspec-results (spec-command) args))
  (set-process-filter proc 'linkify-filter)
  (display-buffer rspec-results))

(defun redo-spec (redo)
  (let ((redo-cmd (eval (append '(lambda () (interactive)) (list redo)))))
    (global-set-key (kbd "M-s") redo-cmd)))
